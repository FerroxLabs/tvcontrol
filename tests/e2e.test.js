...
val = array.get(a, 5)`;
  ...
  describe('UI Automation', () => {
    ...
    it('ui_open_panel — open/close pine-editor', async () => {
      ...
      // Close — guard for API drift (TV 3.1.0 removed hideWidget in some builds)
      await evaluate(`
        (function() {
          var bwb = ${BOTTOM_BAR};
          if (typeof bwb.hideWidget === 'function') { bwb.hideWidget('pine-editor'); return 'hideWidget'; }
          if (typeof bwb.closeWidget === 'function') { bwb.closeWidget('pine-editor'); return 'closeWidget'; }
  ...
  describe('Replay Mode', () => {
    ...
    it('replay_stop — return to realtime', async () => {
      ...
      // TV 3.1.0: stopReplay() alone doesn't fully exit replay; goToRealtime()
      // is what actually transitions back. goToRealtime() internally re-calls
      // stopReplay() which can assert if state is inconsistent, so wrap
      // every call defensively and give the UI time to settle.
      await evaluate(`
        (function() {
          try { ${REPLAY_API}.stopReplay(); } catch(e) {}
          try { if (typeof ${REPLAY_API}.goToRealtime === 'function') ${REPLAY_API}.goToRealtime(); } catch(e) {}
          try { if (typeof ${REPLAY_API}.hideReplayToolbar === 'function') ${REPLAY_API}.hideReplayToolbar(); } catch(e) {}
        })()
      `);
      await sleep(1500);
