...
export function registerBatchTools(server) {
  server.tool('batch_run', 'Run an action across multiple symbols and/or timeframes', {
    ...
    action: z.enum(['screenshot', 'get_ohlcv', 'get_strategy_results', 'get_study_values']).describe('Action to run for each symbol/timeframe. get_study_values reads every visible indicator per symbol and is the one to use for universe scans.'),
